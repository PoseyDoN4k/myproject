document.addEventListener('DOMContentLoaded', () => {
    ...
    const showMessage = (message, success = true) => {
        ...
        shopMessage.style.backgroundColor = success ? 'rgba(0, 128, 0, 0.8)' : 'rgba(128, 0, 0, 0.8)';
    ...
    const buySet = (setName, totalPrice) => {
        if (playerGold >= totalPrice) {
            ...
            const setItems = document.querySelectorAll(`.equipmentSet:has(h2:contains('${setName}')) .equipmentItems li strong`);
            ...
            const setButtons = document.querySelectorAll(`.equipmentSet:has(h2:contains('${setName}')) button`);
